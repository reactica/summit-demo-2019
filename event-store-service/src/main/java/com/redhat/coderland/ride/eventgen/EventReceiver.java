...
public class EventReceiver {
    ...
    private Completable startUserInLineReceiver() {
        return Completable.fromAction( () -> {
            LOGGER.debug("STARTING USER RECEIVER LISTENING TO LOCAL BUS {}",Event.USER_IN_LINE);
            vertx.eventBus().<JsonObject>localConsumer(Event.USER_IN_LINE, message -> {
                LOGGER.debug("RECEIVED USER IN LINE EVENT");
                ...
                user.putInQueue();
                LOGGER.debug(">>> RECEIVED {} from {} for ride {}",Event.USER_IN_LINE, user.getName(),user.getRideId());
    ...
    private Completable startRideStartedEventReceiver() {
        return Completable.fromAction(() -> {
            LOGGER.debug("STARTING EVENT RECEIVER LISTENING TO LOCAL BUS {}",Event.RIDE_STARTED);
            vertx.eventBus().<JsonObject>localConsumer(Event.RIDE_STARTED, message -> {
                LOGGER.debug("RECEIVED START RIDE EVENT");
                ...
                LOGGER.debug("Searching for {} users on ride {}",capacity,attraction );
                ...
                LOGGER.debug("Found {} users on in queue for ride {}",users.size(),capacity);
    ...
    private Completable startRideCompletedEventReceiver() {
        return Completable.fromAction( () -> {
            vertx.eventBus().<JsonObject>localConsumer(Event.RIDE_COMPLETED, message -> {
                LOGGER.debug("RECEIVED RIDE COMPLETED EVENT");
                ...
                rideEvent.getRide().getUserOnTheRide().forEach( user -> {
                    vertx.eventBus().send(Event.USER_COMPLETED,JsonObject.mapFrom(new UserCompletedRideEvent(user)));
                });
            });
        });
    }
    ...
    private Completable startUserCompletedEventReceiver() {
        return Completable.fromAction( () -> {
            vertx.eventBus().<JsonObject>localConsumer(Event.USER_COMPLETED, message -> {
                LOGGER.debug("RECEIVED USER COMPLETED EVENT");
                ...
                vertx.setTimer(10*1000, t -> {
                   vertx.eventBus().send(Event.USER_LEAVING,JsonObject.mapFrom(new UserLeavingRideEvent(user)));
                });
            });
        });
    }
    ...
    private Completable startUserOnRideEventReceiver() {
        return Completable.fromAction(() -> {
            vertx.eventBus().<JsonObject>localConsumer(Event.USER_ON_RIDE, message -> {
                ...
                User user = userEvent.getUser();
                user.onRide();
                LOGGER.debug(">>> RECEIVED {} from {} for ride {}", Event.USER_ON_RIDE, user.getName(), user.getRideId());
                cache.putAsync(user.getId(), user);
            });
        });
